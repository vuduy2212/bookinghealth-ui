...
import styles from './ClinicUpdate.module.scss';
...
import { getOneClinic, updateClinic } from '~/service/clinic';
...
function ClinicUpdate() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            setSubmitting(true);
            await updateClinic(axiosJWT, user, clinicId, data);
    ...
    return (
        <ProtectedRoute isAllowed={!!user && user.roleId === 'R1'}>
            <div>
                <HeaderLite title={`Chỉnh sửa thông tin phòng khám`} />
                {loading ? (
                    <LoadingIcon />
                ) : (
